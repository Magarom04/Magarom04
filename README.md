<div align="centeorkr">

<!-- PORTRAIT - generated by scripts/dotify.py -->

<!-- Your portrait will be generated in the next step. -->

<img src="assets/profile-photo.png" width="280" alt="Om Magar">

<br>

<!-- NAME / TAGLINE - animated typing -->

<a href="https://github.com/Magarom04">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=26&duration=2600&pause=900&color=39D353&center=true&vCenter=true&width=560&lines=Om+Magar;AI+%26+Machine+Learning;Computer+Vision+%26+DSA" alt="typing banner">
</a>

<br>

<!-- SOCIALS -->

<a href="https://www.linkedin.com/in/om-magar-5b187437">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>

<a href="https://leetcode.com/u/Om_M/">
  <img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black" alt="LeetCode">
</a>

<img src="https://komarev.com/ghpvc/?username=Magarom04&style=flat&color=39d353&label=profile+views" alt="profile views">

</div>

---

## `~/` whoami

```console
$ cat about.txt
```

Hi, I'm **Om Magar**, a Computer Science & Engineering graduate interested in **Artificial Intelligence, Machine Learning, Computer Vision, and Software Development**.

* Currently building and exploring **AI/ML and Computer Vision systems**
* Solving **Data Structures & Algorithms** problems with a focus on problem solving
* Working with **Python, C++, PyTorch, OpenCV, and Scikit-learn**
* Interested in building practical AI systems that can be deployed and used in the real world

<br>

<div align="center">

## `~/` toolbox

<img src="https://skillicons.dev/icons?i=python,cpp,js,sql,pytorch,tensorflow,opencv,git,github,vscode,docker&perline=11" alt="tech stack">

</div>

---

<div align="center">

## `~/` skill radar

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/radar-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/radar-light.svg">
  <img src="assets/radar-dark.svg" width="450" alt="Om Magar skill radar">
</picture>

</div>

---

<div align="center">

## `~/` contribution calendar

<!-- 3D isometric calendar, regenerated by .github/workflows/metrics.yml -->

<img src="assets/metrics.isocalendar.svg" width="90%" alt="3D isometric contribution calendar">

<br><br>

<!-- Snake eats the contribution graph - .github/workflows/snake.yml -->

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Magarom04/Magarom04/output/snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Magarom04/Magarom04/output/snake.svg">
  <img src="https://raw.githubusercontent.com/Magarom04/Magarom04/output/snake.svg" width="100%" alt="snake eating the contribution graph">
</picture>

</div>

---

<div align="center">

## `~/` the numbers

<!-- Generated by scripts/cards.py into this repo -->

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/card-stats-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/card-stats-light.svg">
  <img src="assets/card-stats-dark.svg" width="480" alt="GitHub statistics">
</picture>

<br>

<img src="assets/metrics.languages.svg" height="165" alt="most used languages">

<br><br>

<img src="assets/metrics.achievements.svg" width="90%" alt="achievements">

</div>

---

<div align="center">

## `~/` selected work

<!-- Cards generated by scripts/cards.py from assets/projects.json -->

<table>

<tr>

<td width="50%" align="center">

<a href="https://github.com/Magarom04/Medical-deepfake-detection">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/card-Medical-deepfake-detection-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/card-Medical-deepfake-detection-light.svg">
  <img src="assets/card-Medical-deepfake-detection-dark.svg" width="420" alt="Medical Deepfake Detection">
</picture>

</a>

</td>

<td width="50%" align="center">

<a href="https://github.com/Magarom04/neetcode-submissions">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/card-neetcode-submissions-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="assets/card-neetcode-submissions-light.svg">
  <img src="assets/card-neetcode-submissions-dark.svg" width="420" alt="NeetCode submissions">
</picture>

</a>

</td>

</tr>

</table>

<br>

<sub>

| project                                                                                   | description                                                                                   | stack                                         |
| ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------- |
| **[Medical Deepfake Detection](https://github.com/Magarom04/Medical-deepfake-detection)** | Hybrid CNN + ViT system for detecting deepfake medical images with explainable predictions    | `Python` `PyTorch` `ViT` `OpenCV` `Streamlit` |
| **[NeetCode Submissions](https://github.com/Magarom04/neetcode-submissions)**             | Structured collection of Data Structures & Algorithms solutions and coding interview practice | `Python` `DSA`                                |

</sub>

</div>

---

<div align="center">

<sub>`01101111 01101101 00100000 01101101 01100001 01100111 01100001 01110010`</sub>

</div>
